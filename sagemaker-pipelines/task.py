...
def test_code():
    ...
    run_command(
        ["uv", "run", "pytest", "tests/", "-v"], cwd="src/training", check=False
    )
...
def trigger_pipeline(pipeline_type: str, environment: str = "staging"):
    ...
    run_command(
        [
            "python",
            "scripts/generate_params_json.py",
            "--pipeline-type",
            pipeline_type,
            "--environment",
            environment,
...
def main():
    ...
    build_parser.add_argument(
        "--service", choices=["training", "inference"], help="Specific service to build"
    )

    # Push command
    push_parser = subparsers.add_parser("push", help="Push images to ECR")
    push_parser.add_argument(
        "--environment", default="staging", help="Target environment"
    )
    ...
    deploy_parser.add_argument(
        "--auto-approve", action="store_true", help="Auto-approve Terraform changes"
    )

    # Trigger command
    trigger_parser = subparsers.add_parser("trigger", help="Trigger pipeline execution")
    trigger_parser.add_argument(
        "pipeline_type", choices=["training", "inference"], help="Pipeline type"
    )
    trigger_parser.add_argument(
        "--environment", default="staging", help="Target environment"
    )
