...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description='Generate SageMaker Pipeline parameters JSON')
    
    parser.add_argument('--pipeline-type', choices=['training', 'inference'], required=True,
    ...
    parser.add_argument('--project-name', default='ml-platform',
                       help='Project name')
    ...
    parser.add_argument('--inference-input-path', type=str,
    ...
    parser.add_argument('--dry-run', action='store_true',
                       help='Print parameters without writing file')
...
def generate_training_parameters(args: argparse.Namespace, account_id: str) -> Dict[str, Any]:
    ...
    training_data_path = args.training_data_path or f"s3://{bucket_name}/datasets/training/"
...
def generate_inference_parameters(args: argparse.Namespace, account_id: str) -> Dict[str, Any]:
    """Generate parameters for inference pipeline"""
    timestamp = datetime.utcnow().strftime("%Y%m%d-%H%M%S")
    
    # Default S3 paths if not provided
    bucket_name = f"mlflow-{args.environment}-mlflow-artifacts-zali-{args.environment}"
    inference_input_path = args.inference_input_path or f"s3://{bucket_name}/inference/input/"
...
def generate_eventbridge_rule_json(args: argparse.Namespace, parameters: Dict[str, Any]) -> Dict[str, Any]:
    """Generate EventBridge rule configuration"""
    schedule_expression = "cron(0 2 * * ? *)" if args.pipeline_type == "training" else "cron(0 6 * * ? *)"
        ...
        "Targets": [
            {
                ...
                "SageMakerPipelineParameters": {
                    "PipelineParameterList": {k: str(v) for k, v in parameters.items() if isinstance(v, (str, int, float))}
                }
            }
        ],
        "Tags": [
...
def main():
    ...
    if not args.output_file:
        timestamp = datetime.utcnow().strftime("%Y%m%d-%H%M%S")
        args.output_file = f"parameters/{args.pipeline_type}-{args.environment}-{timestamp}.json"
    ...
    if args.dry_run:
        ...
        print(f"1. Review and customize the generated JSON files")
        print(f"2. Use with AWS CLI: aws sagemaker start-pipeline-execution --cli-input-json file://{args.output_file}")
        print(f"3. Or deploy via Terraform using the rule configuration")
